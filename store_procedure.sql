...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 14, 2019 at 08:07 AM
...
SELECT instruments.* FROM instruments LEFT JOIN student_instrument ON instruments.InstrumentID = student_instrument.InstrumentID WHERE StudentID IS NULL$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `BorrowedInstruments` ()  NO SQL
SELECT students.*, instruments.*, student_instrument.CheckOutDate, student_instrument.CheckInDate FROM students INNER JOIN student_instrument ON student_instrument.StudentID = students.StudentID INNER JOIN instruments ON student_instrument.InstrumentID = instruments.InstrumentID$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `StudentsWithBorrowCredits` ()  NO SQL
SELECT students.* FROM students LEFT JOIN student_instrument ON students.StudentID = student_instrument.StudentID GROUP BY students.StudentID HAVING COUNT(student_instrument.InstrumentID) < 2$$
